...
if (!isset($_SESSION['latestPosts'])) {
    $_SESSION["latestPosts"] = getWpPosts();
}

$app->group('/translator', function($app) use ($container){
    
    $app->get('/login', "App\Controllers\AuthController:translator_get_login")->add($container->get('csrf'));
    $app->post('/login', "App\Controllers\AuthController:translator_post_login")->add($container->get('csrf'));
    $app->get('/logout', "App\Controllers\AuthController:translator_logout");
